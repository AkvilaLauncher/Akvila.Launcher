...
public class OverviewPageViewModel : PageViewModelBase {
    ...
    internal OverviewPageViewModel(IScreen screen,
        ...
        IAkvilaClientManager? akvilaManager = null,
        ...
        GoProfileCommand = ReactiveCommand.CreateFromObservable(
            () => screen.Router.Navigate.Execute(new ProfilePageViewModel(screen, User, _akvilaManager))
        );
        ...
        _akvilaManager.ProgressChanged.Subscribe(percentage => {
            if (LoadingPercentage != percentage)
                LoadingPercentage = percentage;
        });

        _akvilaManager.ProfilesChanges.Subscribe(LoadProfilesAsync);
        ...
        _maxCountLoaded ??= _akvilaManager.MaxFileCount.Subscribe(count => MaxCount = count);
        _loadedLoaded ??= _akvilaManager.LoadedFilesCount.Subscribe(ChangeLoadProcessDescription);
    ...
    private async Task StartGame() {
        ...
        await ExecuteFromNewThread(async () => {
            try {
                ...
                UpdateProgress(string.Empty, string.Empty, false);
                await _akvilaManager.UpdateDiscordRpcState(
    ...
    private async Task<Process> GenerateProcess(CancellationToken cancellationToken,
        ...
        await _akvilaManager.DownloadNotInstalledFiles(profileInfo.Data, cancellationToken);

        var process = await _akvilaManager.GetProcess(profileInfo.Data, _systemService.GetOsType());
    ...
    private async Task<ResponseMessage<ProfileReadInfoDto?>?> GetProfileInfo() {
        ...
        var profileInfo = await _akvilaManager.GetProfileInfo(localProfile);
    ...
    private async void LoadData() {
        try {
            ...
            await _akvilaManager.LoadDiscordRpc();
            await _akvilaManager.UpdateDiscordRpcState(
                LocalizationService.GetString(ResourceKeysDictionary.DefaultDRpcText));
        }
        catch (TaskCanceledException exception) {
    ...
    private async Task LoadNews() {
        try {
            var news = await _akvilaManager.GetNews();
    ...
    private async Task LoadProfiles() {
        var profilesData = await _akvilaManager.GetProfiles(User.AccessToken);
